...
BEGIN
  DECLARE given BINARY(64);
  DECLARE correct BINARY(64);

  SELECT sha2(CONCAT(password, salt), 256) FROM user_info WHERE user_info.username=username INTO given;
  SELECT password_hash FROM user_info WHERE user_info.username=username INTO correct;
...
CALL sp_add_user('bob', 'ilovefood');
CALL sp_add_user('jimmy', 'ilovebob');
